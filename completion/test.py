...
def test(config, model=None, test_dataloader=None, epoch_idx=-1, validation=False, test_writer=None):
    ...
    completion_loss = loss_util.CompletionLoss(config.dataset.name, loss_func=config.test.loss_func)
        ...
        for model_idx, (taxonomy_id, model_id, data) in enumerate(t):
            ...
            if config.dataset.name  in ['PCN', 'Completion3D']:
                with jt.no_grad():
                    ...
                    loss_total, losses = completion_loss.get_loss(pcds_pred, partial, gt, test=True)
                ...
                if validation:
                    ...
                    choice = [jt.array([1, 1, 1]), jt.array([1, 1, -1]), jt.array([1, -1, 1]),
                              jt.array([-1, 1, 1]),
                              jt.array([-1, -1, 1]), jt.array([-1, 1, -1]), jt.array([1, -1, -1]),
                              jt.array([-1, -1, -1])]
                    num_crop = int(npoints * crop_ratio[config.test.mode])
                for item in choice:
                    partial, _ = misc.seprate_point_cloud(gt, npoints, num_crop, fps_sampler, fixed_points = item)
