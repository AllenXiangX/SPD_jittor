                ...
                for k, v in data.items():
                    if k in objects and k in data:
                        if transform.__class__ in [
                            RandomFlip, RandomRotatePoints, ScalePoints, RandomMirrorPoints
                        ]:
...
class ToTensor(object):
    ...
    def __call__(self, arr):
        ...
        return arr.astype(np.float32)  # jittor.array(arr.copy())
...
class IO:
    ...
    def _write_pcd(cls, file_path, file_content):
        ...
        pc.points = o3d.utility.Vector3dVector(file_content)
