...
from utils.model_utils import SVRLoss, calc_cd
...
def train():
    ...
    print('# encoder parameters:', sum(param.numel() for param in net.encoder.parameters()))
    print('# decoder parameters:', sum(param.numel() for param in net.decoder.parameters()))
    ...
    optimizer = optimizer(net.parameters(),
                          lr=lr, weight_decay=args.weight_decay, betas=betas)
    ...
    for epoch in range(args.start_epoch, args.nepoch):
        ...
        if epoch % args.epoch_interval_to_save == 0:
            save_model(str(log_dir) + '/checkpoints/' + str(epoch) + 'network.pkl', net)
...
def val(net, curr_epoch_num, val_loss_meters, dataloader_test, best_epoch_losses, LOG_FOUT, log_dir, best_cd_l1, best_cd_l2):
    ...
    with jt.no_grad():
        ...
        if avg_cd_l1 < best_cd_l1:
            ...
            save_model(str(log_dir) + '/checkpoints/bestl1_network.pkl', net)
            logging.info("Saving net...")

        if avg_cd_l2 < best_cd_l2:
            best_cd_l2 = avg_cd_l2
            save_model(str(log_dir) + '/checkpoints/bestl2_network.pkl', net)
            logging.info("Saving net...")
