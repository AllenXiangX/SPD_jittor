...
class RotationFunctions(FunctionGenerator):
    ...
    def __call__(self, points):
        rotated_points = nn.bmm(points, self.operator)
        if self.inplace:
...
class Operation(object):
    def __init__(self, points, inplace=True, keep_track=False):
        ...
        self.points = points
        self.inplace = inplace
        self.dim = len(points.shape)
        self.type = self.points.dtype

        if not self.inplace:
            self.points = self.points.clone()
        if self.dim == 2:
            self.points = self.points.unsqueeze(0)
        elif self.dim == 3:
            pass
        else:
    ...
    def scale(self, scale_vector):
        scaling_op = ScaleFunctions(scale_vector.astype(self.type), inplace=self.inplace)
    ...
    def translate(self, translation_vector):
        translation_op = TranslationFunctions(translation_vector.astype(self.type), inplace=self.inplace)
    ...
    def rotate(self, rotation_vector):
        rotation_op = RotationFunctions(rotation_vector.astype(self.type), inplace=self.inplace)
    ...
    def get_3D_rot_matrix(axis, rad_angle):
        ...
        """
        cos_angle = jt.cos(rad_angle)
        sin_angle = jt.sin(rad_angle)
        rotation_matrix = jt.zeros(rad_angle.size(0), 3, 3)
    ...
    def rotate_axis_angle(self, axis, rad_angle, normals=False):
        ...
        if normals:
            rot_matrix = concat([rot_matrix, rot_matrix], dim=2)
...
class Normalization(Operation):
    ...
    def center_pointcloud(self):
        ...
        centroid = jt.mean(self.points, dim=1, keepdims=True)
    ...
    def normalize_unitL2ball(self):
        ...
        scaling_factor_square = jt.max(jt.sum(self.points ** 2, dim=2, keepdims=True), dim=1, keepdims=True)
        scaling_factor = jt.sqrt(scaling_factor_square)
    ...
    def center_bounding_box(self):
        ...
        :return: diameter
        """
        min_vals = jt.min(self.points, 1, keepdims=True)
        max_vals = jt.max(self.points, 1, keepdims=True)
    ...
    def normalize_bounding_box(self, isotropic=True):
        ...
        if isotropic:
            diameter = jt.max(diameter, 2, keepdims=True)
...
class DataAugmentation(Operation):
    ...
    def random_anisotropic_scaling(self, min_val=0.75, max_val=1.25):
        ...
        :return:
        """
        scale = jt.rand(self.points.size(0), 1, self.points.size(2)) * (max_val - min_val) + min_val
        self.scale(scale)
        return
    ...
    def random_axial_rotation(self, axis=0, normals=False, range_rot=360):
        ...
        rad_angle = jt.rand(self.points.size(0)) * 2 * scale_factor - scale_factor
    ...
    def get_random_rotation_matrix(batch_size=1):
        ...
        x = jt.randn(batch_size, 1, 3).double()
        scaling_factor_square = jt.max(jt.sum(x ** 2, dim=2, keepdims=True), dim=1, keepdims=True)
        scaling_factor = jt.sqrt(scaling_factor_square)
        x /= scaling_factor
        x = x.squeeze()
        XX = nn.bmm(x.unsqueeze(2), x.unsqueeze(1))

        # get random angle
        rad_angle = jt.rand(batch_size).double() * 2 * np.pi + np.pi
        cos_angle = jt.cos(rad_angle)
        sin_angle = jt.sin(rad_angle)

        # Compute fat matrix
        rotation_matrix = jt.zeros(rad_angle.size(0), 3, 3).double()
    ...
    def random_translation(self, scale=0.03):
        ...
        :return:
        """
        translation_vector = jt.rand(self.points.size(0), 1, self.points.size(2)) * 2 * scale - scale
    ...
    def random_flips(self, dims=[]):
        ...
        scale_factor = jt.randint(2, (self.points.size(0), 1, self.points.size(2))) * 2 - 1
