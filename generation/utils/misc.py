...
class CheckpointManager(object):
    ...
    def save(self, model, args, score, others=None, step=None):
        ...
        if step is None:
            fname = 'ckpt_%.6f_.pkl' % float(score)
        else:
            fname = 'ckpt_%.6f_%d.pkl' % (float(score), int(step))
    ...
    def load_best(self):
        ...
        ckpt = jt.load(os.path.join(self.save_dir, self.ckpts[idx]['file']))
        return ckpt
    ...
    def load_selected(self, file):
        ckpt = jt.load(os.path.join(self.save_dir, file))
        return ckpt
