...
class FixedPoints(object):
    ...
    def __call__(self, data):
        ...
        if self.replace:
            ...
            choice = concat([
                jt.randperm(num_nodes)
                for _ in range(math.ceil(self.num / num_nodes))
            ], dim=0)[:self.num]

        for key, item in data.items():
            if jt.is_var(item) and item.size(0) == num_nodes and key != 'dense':
...
class LinearTransformation(object):
    ...
    def __call__(self, data):
        for key in self.attr:
            pos = data[key].view(-1, 1) if len(data[key].shape) == 1 else data[key]

            assert pos.shape[-1] == self.matrix.shape[-2], (
                'Node position matrix and transformation matrix have incompatible '
                'shape.')

            data[key] = np.matmul(pos, self.matrix.astype(pos.dtype))
...
class RandomRotate(object):
    ...
    def __call__(self, data):
        ...
        return LinearTransformation(np.array(matrix), attr=self.attr)(data)
...
class AddNoise(object):
    ...
    def __call__(self, data):
        ...
        data['pos'] = data['pos'] + jt.normal(mean=0, std=self.std, size=data['pos'].size())
        return data
...
class AddRandomNoise(object):
    ...
    def __call__(self, data):
        ...
        data[self.key] = data['pos']
        data['pos'] = data['pos'] + jt.normal(mean=0, std=noise_std, size=data['pos'].size())
        return data
...
class AddNoiseForEval(object):
    ...
    def __call__(self, data):
        ...
        for noise_std in self.stds:
            data['noisy_%.2f' % noise_std] = data['pos'] + jt.normal(mean=0, std=noise_std, size=data['pos'].size())
        return data
...
class RandomTranslate(object):
    ...
    def __call__(self, data):
        ...
        for key in self.attr:
            data[key] = data[key] + jt.stack(ts, dim=-1)
...
class Rotate(object):
    ...
    def __call__(self, data):
        ...
        if self.axis == 0:
            ...
            matrix = [[cos, sin, 0], [-sin, cos, 0], [0, 0, 1]]
        return LinearTransformation(jt.array(matrix), attr=self.attr)(data)
