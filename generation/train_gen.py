...
from utils.misc import seed_all, get_logger, str_list, \
    THOUSAND, get_new_log_dir, CheckpointManager, \
    BlackHole, get_linear_scheduler, log_hyperparams
...
def train(it):
    ...
    orig_grad_norm = optimizer.param_groups[0]['grads'][0].norm()
    optimizer.step()
    scheduler.step()

    logger.info('[Train] Iter %04d | Loss %.6f | Grad %.7f | KLWeight %.4f' % (
...
def validate_inspect(it):
    z = jt.randn([args.num_samples, args.latent_dim])
    x = model.sample(z) #, truncate_std=args.truncate_std)
    writer.add_mesh('val/pointcloud', x.numpy(), global_step=it)
    writer.flush()
...
def test(it):
    ...
    for i in tqdm(range(0, math.ceil(args.test_size / args.val_batch_size)), 'Generate'):
        with jt.no_grad():
            z = jt.randn([args.val_batch_size, args.latent_dim]).to(args.device)
            x = model.sample(z)
            gen_pcs.append(x.detach().cpu())
    gen_pcs = concat(gen_pcs, dim=0)[:args.test_size]
