...
sys.path.append('..')
from loss_functions import chamfer_l2_batch as chamfer
...
def emd_approx(sample, ref):
    ...
    emd = jt.zeros([sample.size(0)]).to(sample)
...
def distChamfer(a, b):
    ...
    xx = nn.bmm(x, x.transpose(2, 1))
    yy = nn.bmm(y, y.transpose(2, 1))
    zz = nn.bmm(x, y.transpose(2, 1))
    diag_ind = jt.arange(0, num_points).to(a).long()
...
def EMD_CD(sample_pcs, ref_pcs, batch_size, accelerated_cd=False, reduced=True):
    ...
    if reduced:
        cd = concat(cd_lst).mean()
        emd = concat(emd_lst).mean()
    else:
        cd = concat(cd_lst)
        emd = concat(emd_lst)
...
def _pairwise_EMD_CD_(sample_pcs, ref_pcs, batch_size,
    ...
    for sample_b_start in iterator:
        ...
        cd_lst = concat(cd_lst, dim=1)
        emd_lst = concat(emd_lst, dim=1)
        all_cd.append(cd_lst)
        all_emd.append(emd_lst)

    all_cd = concat(all_cd, dim=0)  # N_sample, N_ref
    all_emd = concat(all_emd, dim=0)  # N_sample, N_ref
...
def knn(Mxx, Mxy, Myy, k, sqrt=False):
    n0 = Mxx.size(0)
    n1 = Myy.size(0)
    label = concat((jt.ones(n0), jt.zeros(n1))).to(Mxx)
    M = concat([
        concat((Mxx, Mxy), 1),
        concat((Mxy.transpose(0, 1), Myy), 1)], 0)
    if sqrt:
        M = M.abs().sqrt()
    INFINITY = float('inf')
    val, idx = (M + jt.diag(INFINITY * jt.ones(n0 + n1).to(Mxx))).topk(
        k, 0, False)

    count = jt.zeros(n0 + n1).to(Mxx)
    for i in range(0, k):
        count = count + label.gather(0, idx[i])
    pred = jt.greater_equal(count, (float(k) / 2) * jt.ones(n0 + n1).to(Mxx)).float()
    ...
    s.update({
        ...
        'acc': jt.equal(label, pred).float().mean(),
    })
    return s
...
def lgan_mmd_cov(all_dist):
    ...
    min_idx, min_val_fromsmp = jt.argmin(all_dist, dim=1)
    _, min_val = jt.argmin(all_dist, dim=0)
    mmd = min_val.mean()
    mmd_smp = min_val_fromsmp.mean()
    cov = float(min_idx.unique().view(-1).size(0)) / float(N_ref)
    cov = jt.array(cov).to(all_dist)
    return {
        'lgan_mmd': mmd,
        'lgan_cov': cov,
...
if __name__ == '__main__':
    a = jt.randn([16, 2048, 3])
    b = jt.randn([16, 2048, 3])
