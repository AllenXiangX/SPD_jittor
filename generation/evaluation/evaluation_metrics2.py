...
from emd_torch.emd import earth_mover_distance
...
def emd_loss(p1, p2):
    n = p2.shape[1]
    return earth_mover_distance(p1, p2, transpose=False) / float(n)
...
def EMD_CD(sample_pcs, ref_pcs, batch_size, accelerated_cd=False, reduced=True):
    ...
    for b_start in tqdm(iterator, desc='EMD-CD'):
        ...
        emd_batch = emd_loss(sample_batch, ref_batch)
