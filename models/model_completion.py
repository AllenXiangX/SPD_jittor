...
from .utils import PointNet_SA_Module_KNN, MLP_Res, FurthestPointSampler, Transformer
from .SPD import SPD
...
class FeatureExtractor(nn.Module):
    ...
    def execute(self, point_cloud):
        ...
        """
        l0_xyz = point_cloud
        l0_points = point_cloud.permute(0, 2, 1)
...
class Decoder(nn.Module):
    def __init__(self, dim_feat=512, num_pc=256, num_p0=512,
        ...
        self.sampler = FurthestPointSampler(self.num_p0)

    def execute(self, feat, partial, return_P0=False):
        """
        Args:
        ...
        arr_pcd.append(pcd)
        pcd = self.sampler(concat([pcd, partial], 1))
        if return_P0:
            arr_pcd.append(pcd)
        K_prev = None
...
class SnowflakeNet(nn.Module):
    ...
    def execute(self, point_cloud, return_P0=False):
        """
        Args:
            point_cloud: (B, N, 3)
        """
        feat = self.feat_extractor(point_cloud)
        out = self.decoder(feat, point_cloud, return_P0=return_P0)
        return out
