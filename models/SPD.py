...
class SPD(nn.Module):
    def __init__(self, dim_feat=512, up_factor=2, i=0, radius=1, bounding=True, global_feat=True):
        ...
        self.ps = nn.ConvTranspose(self.ps_dim, 128, (up_factor, 1), (up_factor, 1), bias=False)   # point-wise splitting

        self.up_sampler = nn.Upsample(scale_factor=(up_factor, 1))
    ...
    def execute(self, pcd_prev, feat_global=None, K_prev=None):
        """
        Args:
        ...
        """
        b, n_prev, _ = pcd_prev.shape
        pcd_b3n = pcd_prev.permute(0, 2, 1)
        feat_1 = self.mlp_1(pcd_b3n)
        feat_1 = concat([feat_1,
                            jt.max(feat_1, 2, keepdims=True).repeat((1, 1, feat_1.size(2))),
        ...
        feat_child = self.ps(feat_child.unsqueeze(3)).squeeze(3)  # (B, 128, N_prev * up_factor)
        H_up = self.up_sampler(H.unsqueeze(3)).squeeze(3)
        K_curr = self.mlp_delta_feature(concat([feat_child, H_up], 1))

        delta = self.mlp_delta(nn.relu(K_curr))
        if self.bounding:
            delta = jt.tanh(delta) / self.radius**self.i  # (B, 3, N_prev * up_factor)

        pcd_child = self.up_sampler(pcd_b3n.unsqueeze(3)).squeeze(3)
        pcd_child = pcd_child + delta

        return pcd_child.permute(0, 2, 1), K_curr